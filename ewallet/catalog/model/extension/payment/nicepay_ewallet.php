...
class ModelExtensionPaymentnicepayEWallet extends Model {
  ...
  public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('nicepay_ewallet_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('nicepay_ewallet_total') > 0 && $this->config->get('nicepay_ewallet_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('nicepay_ewallet_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
      $method_data = array(
        'code'       => 'nicepay_ewallet',
        'title'      => $this->config->get('nicepay_ewallet_display_name'),
				'sort_order' => $this->config->get('nicepay_ewallet_sort_order'),
				'terms' 	 => ''
    );
    }
