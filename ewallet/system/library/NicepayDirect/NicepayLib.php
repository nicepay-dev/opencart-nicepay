...
class NicepayLib {
    ...
    public function merchantToken() {
        // SHA256( Concatenate(timeStamp+iMid + referenceNo + amt + merchantKey) )
        return hash('sha256', $this->get('timeStamp').$this->get('iMid').$this->get('referenceNo').$this->get('amt').$this->merchantKey);
    }

    public function merchantTokenRedirect() {
        // SHA256( Concatenate(timeStamp+iMid + referenceNo + amt + merchantKey) )
        return hash('sha256', $this->get('timeStamp').$this->get('iMid').$this->get('referenceNo').$this->get('amt').$this->merchantKey);
    }
    ...
    public function requestVA() {
        ...
        // $this->set('callBackUrl', $this->dbProcessUrl);
        ...
        $this->resultData = $this->request->apiRequest($this->requestData, $this->request->apiUrl);
        unset($this->requestData);
        return $this->resultData;
    }

    // Charge Credit Card
    public function reqChargeCard() {
        // Populate data
        if ($this->get('iMid')  == "") {
            $this->set('iMid', $this->iMid);
        }
        $this->set('merchantToken', $this->merchantTokenRedirect());
        $this->set('dbProcessUrl', $this->dbProcessUrl);
        $this->set('callBackUrl', $this->callBackUrl);
        if ($this->get('instmntMon')  == "") {
            $this->set('instmntMon', '1');
        }
        if ($this->get('instmntType')  == "") {
        ...
        $this->checkParam('merchantToken', '28');
                ...
                // Send Request
        $this->request->operation('requestTRX');
        ...
        return $this->resultData;
         ...
         // Charge Credit Card
    public function reqChargeEwallet() {
        // Populate data
        // print_r("MASUK SINI ISI I_MID ".$this->iMid); 
        if ($this->get('iMid')  == "") {
            $this->set('iMid', $this->iMid);
        }
        $this->set('merchantToken', $this->merchantToken());
        ...
        $this->resultData = $this->request->apiRequestRedV2($this->requestData, $this->request->apiUrl);
        unset($this->requestData);
        return $this->resultData;
    }


         public function reqChargePayloan() {
        // Populate data
        // print_r("asd"); exit;
        if ($this->get('iMid')  == "") {
            $this->set('iMid', $this->iMid);
        ...
        $this->checkParam('mitraCd', '29');
                ...
                // Send Request
        $this->request->operation('requestPay');
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequestRedV2($this->requestData, $this->request->apiUrl);
        unset($this->requestData);
        // print_r($this->resultData); exit();
        return $this->resultData;
    }
		...
		public function requestCVS() {
		  ...
		  // $this->checkParam('callBackUrl', '23');
		  $this->checkParam('dbProcessUrl', '8');
		  $this->checkParam('vat', '24');
		  $this->checkParam('fee', '25');
		  $this->checkParam('notaxAmt', '26');
		  $this->checkParam('description', '27');
    ...
    public function checkPaymentStatus($tXid, $referenceNo, $amt) {
        ...
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequestV1($this->requestData);
        unset($this->requestData);
        return $this->resultData;
    }
    ...
    public function cancelVA($tXid, $amt) {
        ...
        // Send Request
        $this->request->operation('cancelTRX');
        $this->request->openSocket();
