...
// include_once ('NicepayLoggerVA.php');
include_once ('NicepayConfigVA.php');

class NicepayRequestorVA {
    public $sock = 0;
    public $apiUrl;
    public $port = 443;
    public $status;
    ...
    public $X_CLIENT_KEY;
    public $X_TIMESTAMP;
    // public $stringToSign = $X_CLIENT_KEY."|".$X_TIMESTAMP;
    public $private_key;
    public $binary_signature;
    public $algo = "SHA256";
    ...
    public function operation($type) {
        if ($type == "requestVA") {
            // $this->apiUrl = NICEPAY_REG_URL_VA;
            // $this->apiUrl = NICEPAY_REQ_ACCESS_TOKEN_URL;

        // } else if ($type == "creditCard") {
            // $this->apiUrl = NICEPAY_REQ_CC_URL;
        } else if ($type == "checkPaymentStatus") {
            $this->apiUrl = NICEPAY_ORDER_STATUS_URL_VA;
    ...
    public function openSocket() {
        ...
        $tryCount = 0;
        if (! $this->sock = @fsockopen ("ssl://".$host, $this->port, $errno, $errstr, NICEPAY_TIMEOUT_CONNECT_VA)) {
            while ($tryCount < 5) {
                if ($this->sock = @fsockopen("ssl://".$host, $this->port, $errno, $errstr, NICEPAY_TIMEOUT_CONNECT_VA)) {
                    return true;
                }
    ...
    public function apiRequest($data) {
        ...
        if($this->sock) {
            ...
            while ( ! feof ($this->sock ) && ! $timeout) {
                ...
                if ($diff >= NICEPAY_TIMEOUT_READ_VA) {
                    $timeout = true;
                }
                if ($IsHeader) {
            ...
            if ($timeout) {
                $this->errorcode = NICEPAY_READ_TIMEOUT_ERR_VA;
