...
class NicepayLoggerVA {
    ...
    function StartLog($dir, $mid) {
        ...
        $logfile = $dir. "/logs/".NICEPAY_PROGRAM_VA."_".$this->type."_".$mid."_".date("ymd").".log";
        $this->handle = fopen( $logfile, "a+" );
        if(!$this->handle) {
            return false;
        }
        
        $this->WriteLog("START ".NICEPAY_PROGRAM_VA." ".$this->type." (V".NICEPAY_VERSION_VA."B".BUILDDATE."(OS:".php_uname('s').php_uname('r').",PHP:".phpversion()."))");
        return true;
    }
